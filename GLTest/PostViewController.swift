...
class PostViewController: UIViewController , UITextFieldDelegate ,
    ...
    //テキストがnullの時に，投稿すると，アラートを出す
    func nullTextAlert(textView: UITextView) -> Bool{
        if(textView.text == "" && imageData == nil){
            let alert : UIAlertController =
            UIAlertController(title: "エラー", message: "画像・動画の添付，またはメッセージ入力をしてください．",
    ...
    //140文字以上の時にアラートを出す
    func overTextCountAlert(textView: UITextView) -> Bool{
        if count(textView.text) >= 140 {
            let alert : UIAlertController =
            UIAlertController(title: "エラー", message: "メッセージが140文字を超えています．",
                preferredStyle:UIAlertControllerStyle.Alert)
            let cancelAction : UIAlertAction =
            UIAlertAction(title: "OK", style: UIAlertActionStyle.Default,
                handler: {(action:UIAlertAction!) -> () in
    ...
    @IBAction func pushedPostButton(sender: AnyObject) {
        ...
        //メッセージ確認
        if(!nullTextAlert(textView) || !overTextCountAlert(textView)) {
            return
        }
